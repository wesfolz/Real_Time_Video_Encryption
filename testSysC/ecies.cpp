...
#include "ecies.h"
	...
	char * ecies_encrypt(char *key, unsigned char *data, size_t length) {
		...
		else if (!(cryptex = ((char*)secure_alloc(envelope_length, EVP_MD_size(ECIES_HASHER), length, length + (length % block_length ? (block_length - (length % block_length)) : 0)))) ){
		...
		if (HMAC_Init_ex(&hmac, envelope_key + key_length, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(&hmac, (const unsigned char*)secure_body_data(cryptex), secure_body_length(cryptex))
	...
	unsigned char * ecies_decrypt(char *key, char *cryptex, size_t *length) {
		...
		// At the moment we are generating the hash using encrypted data. At some point we may want to validate the original text instead.
		if (HMAC_Init_ex(&hmac, envelope_key + key_length, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(&hmac, (const unsigned char*)secure_body_data(cryptex),
